...
{
    float4 base_color = textures.Sample(samper_state, input.t);

	//float4 abledo = g_txTex.SampleLevel(g_SampleWrap, input.t, input.lod);
	//// Light 
	float bright = max(0.2f, dot(input.n.xyz, -direction));
